...
class PairingKey(db.Model):
	...
	uuid = db.Column(db.String(16), unique=True, nullable=False) # UUID is a 128-bit (16-byte) value
...
class Action(db.Model):
	...
	id = db.Column(db.Integer, primary_key=True)
	microbot_id = db.Column(db.Integer, ForeignKey("microbots.id"), nullable=False)
	microbot = relationship("Microbot", back_populates="actions", uselist=False)

	prev_action_id = db.Column(db.Integer, ForeignKey("actions.id"), nullable=True)
	
	next_actions = relationship('Action', backref=backref('prev_action', remote_side=[id]))

	prev_action_delay = db.Column(db.Float, nullable=False, default=0.0)

	retries_left = db.Column(db.Integer, nullable=False, default=5) # Number of times this command will be retried if call raises an exception
	...
	action_args = db.Column(db.PickleType(), default=(), nullable=False) # (<args>, <kwargs>)
